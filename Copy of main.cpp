...
HRESULT CFireFlies::InitDeviceObjects()
{
	...
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\firefly.png"), &m_ptexFirefly)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}        		
...
HRESULT CMetaBall::Render()
{
	...
	g_pd3dDevice->SetTexture( 0, m_ptexMetaBall );
	...
	D3DXMatrixRotationYawPitchRoll( &matRot, 0.9f*g_fDTime, 0.0f*g_fDTime, 0.0f*g_fDTime );
...
HRESULT CScene08::Animate( FLOAT fLocalTime )
{
	...
	g_cam.SetViewParams(D3DXVECTOR3(    0.0f,  50.0f,  -80.0f),
...
HRESULT CScene08::Render()
{
	...
	for ( INT i=0; i<750; i++ )
	{
		fOffset = (FLOAT)i / (750-1) * D3DX_PI*2;
		D3DXMatrixRotationYawPitchRoll( &matRot, -1.5f*(g_fDTime+fOffset), -0.0f*(g_fDTime+fOffset), -2.0f*(g_fDTime+fOffset) );
...
HRESULT CScene08::InitDeviceObjects()
{
	...
	if ( FAILED(hr = m_pmeshStar->Create( g_pd3dDevice, "data\\meshes\\star.x")) )
	{
	...
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, "data\\bitmaps\\envmap.jpg", &m_ptexStar)) )
	{
		DisplayErrorMsg2( hr, NULL );
		return hr;
	}
...
HRESULT InitApp()
{
	...
	g_ptexWhatIf			= NULL;		
	g_ptexWereToBlaim		= NULL;		
	g_ptexHumansAre			= NULL;		
	g_ptexPsychologically	= NULL;		
	g_ptexTheSun			= NULL;
	g_ptexTheEarth			= NULL;
	g_ptexWillBlaim			= NULL;
	g_ptexUsAlone			= NULL;
	...
	// Scene 99
	g_pmeshTorus	= new CD3DMesh();
	g_bTexmodePlain	= FALSE;
	g_pvbTorus		= NULL;
	g_pibTorus		= NULL;
	g_ptexTorus		= NULL;
...
//-----------------------------------------------------------------------------
	...
	if ( FAILED(hr = g_pd3dDevice->CreateVertexBuffer( 4*sizeof(TLTVERTEX), 0, D3DFVF_TLTVERTEX, D3DPOOL_MANAGED, &g_pvbTexture)) )
	{
		DisplayErrorMsg2( hr, NULL );
		return hr;
	}
...
//-------------------------------------------------------------------------
	...
	if ( FAILED(hr = D3DXCreateTextureFromFile( g_pd3dDevice, "data\\bitmaps\\loadingscreen.png", &g_ptexLoadScreen)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}	
	...
	// Scene 04
#ifdef PLAY_SCENE04	
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, "data\\bitmaps\\envmap2.jpg", &g_metaBall.m_ptexMetaBall )) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}			
	...
	// Scene 06
#ifdef PLAY_SCENE06
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, "data\\bitmaps\\forest.jpg", &g_ptexSplash2)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}
	...
	// Scene 07
#ifdef PLAY_SCENE07
/*	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, "data\\bitmaps\\credit1.jpg", &g_ptexCredit1)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}
*/
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, "data\\bitmaps\\credit2.jpg", &g_ptexCredit2)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, "data\\bitmaps\\credit3.jpg", &g_ptexCredit3)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, "data\\bitmaps\\hello.jpg", &g_ptexHello)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}
	...
	// Scene 99
#ifdef PLAY_SCENE99
	if ( FAILED(hr = g_pmeshTorus->Create( g_pd3dDevice, "data\\meshes\\torus.x")) )
	{
		DisplayErrorMsg2( hr, D3DAPPERR_MEDIANOTFOUND );
		return hr;
	}
	...
	if ( g_bTexmodePlain )
		g_pmeshTorus->SetFVF( g_pd3dDevice, D3DFVF_NTVERTEX );
	else
		g_pmeshTorus->SetFVF( g_pd3dDevice, D3DFVF_NVERTEX );
	
	g_pmeshTorus->UseMeshMaterials( FALSE );
	g_pmeshTorus->GetSysMemMesh()->GetVertexBuffer( &g_pvbTorus );
	g_pmeshTorus->GetSysMemMesh()->GetIndexBuffer( &g_pibTorus );
	CenterMesh( g_pmeshTorus, g_pvbTorus );
	SizeMesh( g_pmeshTorus, g_pvbTorus, 70.0f );
#endif
...
// us Alone
	...
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\whatif.png"), &g_ptexWhatIf)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\wereblaim.png"), &g_ptexWereToBlaim)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}
	
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\humansare.png"), &g_ptexHumansAre)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}
	
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\psychologically.png"), &g_ptexPsychologically)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\thesun.png"), &g_ptexTheSun)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\theearth.png"), &g_ptexTheEarth)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\willblaim.png"), &g_ptexWillBlaim)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\usalone.png"), &g_ptexUsAlone)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\iamfeeling.png"), &g_ptexIAmFeeling)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\inaviolated.png"), &g_ptexInAViolated)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\icannolonger.png"), &g_ptexICanNoLonger)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\thethings.png"), &g_ptexTheThings)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\ihaveforsaken.png"), &g_ptexIHaveForsaken)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}
	...
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\heed.png"), &g_ptexHeed)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\presents.png"), &g_ptexPresents)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}
	
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\ourfirst.png"), &g_ptexOurFirst)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}
	
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\demoever.png"), &g_ptexDemoEver)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}
	...
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, "data\\bitmaps\\splash.jpg", &g_ptexSplash1)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, "data\\bitmaps\\bluewater.jpg", &g_ptexWater )) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}
...
HRESULT InvalidateDeviceObjects()
{
	...
	SAFE_RELEASE( g_ptexWhatIf );
	SAFE_RELEASE( g_ptexWereToBlaim );
	SAFE_RELEASE( g_ptexHumansAre );
	SAFE_RELEASE( g_ptexPsychologically );
	SAFE_RELEASE( g_ptexTheSun );
	SAFE_RELEASE( g_ptexTheEarth );
	SAFE_RELEASE( g_ptexWillBlaim );
	SAFE_RELEASE( g_ptexUsAlone );
	...
	// Scene 99
#ifdef PLAY_SCENE99
	g_pmeshTorus->InvalidateDeviceObjects();
	g_pmeshTorus->Destroy();
	SAFE_DELETE( g_pmeshTorus );

	SAFE_RELEASE( g_pvbTorus );
	SAFE_RELEASE( g_pibTorus );
	SAFE_RELEASE( g_ptexTorus );
#endif
...
#ifdef PLAY_SCENE99
		if ( iOnlyPlayScene == 99 )
			if ( FAILED(hr = AScene99(g_fDTime)) )
				return hr;
#endif
...
HRESULT AScene01( FLOAT fTime )
{
	...
	FLOAT fText1X1 = 130.0f;	FLOAT fText2X1 = 220.0f;
	FLOAT fText1X2 = 200.0f;	FLOAT fText2X2 = 120.0f;
	FLOAT fText1Y1 = 140.0f;	FLOAT fText2Y1 = 190.0f;
	FLOAT fText1Y2 = 170.0f;	FLOAT fText2Y2 = 250.0f;

	FLOAT fText3X1 = 200.0f;	FLOAT fText4X1 = 250.0f;
	FLOAT fText3X2 = 280.0f;	FLOAT fText4X2 = 180.0f;
	FLOAT fText3Y1 = 150.0f;	FLOAT fText4Y1 = 195.0f;
	FLOAT fText3Y2 = 190.0f;	FLOAT fText4Y2 = 250.0f;

	FLOAT fText5X1 = 300.0f;	
	FLOAT fText5X2 = 350.0f;	
	...
	if ( fLocalTime >= fTStartText1 && fLocalTime < fTEndText1 )
	{
		...
		if ( fLocalTime < fTStartText1+1.0f )
			ShowTexture( g_ptexIAmFeeling, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(0.0f, 180.0f, fTStartText1, fLocalTime, fTStartText1+1.0f) );
		else
			ShowTexture( g_ptexIAmFeeling, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(180.0f, 0.0f, fTEndText1-1.0f, fLocalTime, fTEndText1) );
	}
	...
	if ( fLocalTime >= fTStartText2 && fLocalTime < fTEndText2)
	{
		...
		if ( fLocalTime < fTStartText2+1.0f )
			ShowTexture( g_ptexInAViolated, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(0.0f, 180.0f, fTStartText2, fLocalTime, fTStartText2+1.0f) );
		else
			ShowTexture( g_ptexInAViolated, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(180.0f, 0.0f, fTEndText2-1.0f, fLocalTime, fTEndText2) );
	}			
	...
	if ( fLocalTime >= fTStartText3 && fLocalTime < fTEndText3)
	{
		...
		if ( fLocalTime < fTStartText3+1.0f )
			ShowTexture( g_ptexICanNoLonger, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(0.0f, 180.0f, fTStartText3, fLocalTime, fTStartText3+1.0f) );
		else
			ShowTexture( g_ptexICanNoLonger, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(180.0f, 0.0f, fTEndText3-1.0f, fLocalTime, fTEndText3) );
	}			
	...
	if ( fLocalTime >= fTStartText4 && fLocalTime < fTEndText4)
	{
		...
		if ( fLocalTime < fTStartText4+1.0f )
			ShowTexture( g_ptexTheThings, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(0.0f, 180.0f, fTStartText4, fLocalTime, fTStartText4+1.0f) );
		else
			ShowTexture( g_ptexTheThings, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(180.0f, 0.0f, fTEndText4-1.0f, fLocalTime, fTEndText4) );
	}			
	...
	if ( fLocalTime >= fTStartText5 && fLocalTime < fTEndText5)
	{
		...
		if ( fLocalTime < fTStartText5+1.0f )
			ShowTexture( g_ptexIHaveForsaken, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(0.0f, 180.0f, fTStartText5, fLocalTime, fTStartText5+1.0f) );
		else
			ShowTexture( g_ptexIHaveForsaken, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(180.0f, 0.0f, fTEndText5-1.0f, fLocalTime, fTEndText5) );
	}			
	...
	if ( fLocalTime >= fTStartText6 && fLocalTime < fTEndText6 )
	{
		...
		if ( fLocalTime < fTStartText6+1.0f )
			ShowTexture( g_ptexHeed, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(0.0f, 180.0f, fTStartText6, fLocalTime, fTStartText6+1.0f) );
		else
			ShowTexture( g_ptexHeed, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(180.0f, 0.0f, fTEndText6-1.0f, fLocalTime, fTEndText6) );
	}			
	...
	if ( fLocalTime >= fTStartText7 && fLocalTime < fTEndText7 )
	{
		...
		if ( fLocalTime < fTStartText7+1.0f )
			ShowTexture( g_ptexPresents, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(0.0f, 180.0f, fTStartText7, fLocalTime, fTStartText7+1.0f) );
		else
			ShowTexture( g_ptexPresents, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(180.0f, 0.0f, fTEndText7-1.0f, fLocalTime, fTEndText7) );
	}			
	...
	if ( fLocalTime >= fTStartText8 && fLocalTime < fTEndText8 )
	{
		...
		if ( fLocalTime < fTStartText8+1.0f )
			ShowTexture( g_ptexOurFirst, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(0.0f, 180.0f, fTStartText8, fLocalTime, fTStartText8+1.0f) );
		else
			ShowTexture( g_ptexOurFirst, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(180.0f, 0.0f, fTEndText8-1.0f, fLocalTime, fTEndText8) );
	}			
	...
	if ( fLocalTime >= fTStartText9 && fLocalTime < fTEndText9 )
	{
		...
		if ( fLocalTime < fTStartText9+1.0f )
			ShowTexture( g_ptexDemoEver, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(0.0f, 180.0f, fTStartText9, fLocalTime, fTStartText9+1.0f) );
		else
			ShowTexture( g_ptexDemoEver, frcText.left, frcText.top, 800.0f, 100.0f, LinearChange(180.0f, 0.0f, fTEndText9-1.0f, fLocalTime, fTEndText9) );
	}			
...
HRESULT AScene07( FLOAT fTime )
{
	...
	if ( g_fDTime-TIME_SCENE01 >= 180.0f + 49.5f )
	{
...
HRESULT AScene11( FLOAT fTime )
{
	...
	static BOOL bFirstTime = TRUE;
	static FLOAT fStartTime = fTime;
	FLOAT fLocalTime = (fTime - TIME_SCENE11);
...
HRESULT AScene99( FLOAT fTime )
{
	...
	static BOOL bFirstTime = TRUE;
	static FLOAT fStartTime = fTime;
	FLOAT fLocalTime = (fTime - TIME_SCENE99);

	if ( bFirstTime )
	{
		if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, "texture.png"/*"data\\bitmaps\\envmap2.jpg"*/, &g_ptexTorus )) )
		{
			DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
			return hr;
		}

		g_pmeshTorus->RestoreDeviceObjects( g_pd3dDevice );

		D3DXComputeNormals( g_pmeshTorus->GetSysMemMesh() );

		g_dirLight.Direction = D3DXVECTOR3( 0.0f, -1.0f, 0.0f);
		g_pd3dDevice->SetLight( LIGHT_DIRECTIONAL, &g_dirLight );


		g_cam.SetViewParams(D3DXVECTOR3(    0.0f,  10.0f,  -200.0f),
							D3DXVECTOR3(    0.0f,   0.0f,    0.0f),
							D3DXVECTOR3(    0.0f,   1.0f,    0.0f) );
	...
	D3DXMATRIX matRot;
	D3DXMatrixRotationYawPitchRoll( &matRot, 1.0f*fLocalTime, 1.0f*fLocalTime, 1.0f*fLocalTime );
	g_pd3dDevice->SetTransform( D3DTS_WORLD, &matRot );
	...
	g_pd3dDevice->SetTexture( 0, g_ptexTorus );
	g_pmeshTorus->Render( g_pd3dDevice );
...
#ifdef PLAY_SCENE03
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, "data\\bitmaps\\sentence.png", &m_ptexText1 )) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}		

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, "data\\bitmaps\\scene4overlay.png", &m_ptexOverlay )) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}			

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, "data\\bitmaps\\textscroll.png", &m_ptexOverlayText )) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}			
...
HRESULT CTunnel::Animate( FLOAT fLocalTime )
{
	...
	fProgress = (fLocalTime-fLastTime) * 0.5f;
	...
	RotateTexCoords( m_pvbTunnel3, m_pibTunnel3, 0.15f, 155.0f );
...
HRESULT CTunnel::InitDeviceObjects()
{
	...
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\bluewater8.jpg"), &m_ptexTunnel)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\plasma.png"), &m_ptexPlasma)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}

	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\plasma2.png"), &m_ptexPlasma2)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}
...
HRESULT CTunnel2::FlyCamera( FLOAT fProgress, BOOL bWaypointChanged )
{
	...
	{
		D3DXVECTOR3(+TUNNEL2_WILDCAM_RADIUS, -TUNNEL2_WILDCAM_RADIUS, +15.0f),
		D3DXVECTOR3(-TUNNEL2_WILDCAM_RADIUS, +TUNNEL2_WILDCAM_RADIUS, +15.0f),
		D3DXVECTOR3(+TUNNEL2_WILDCAM_RADIUS, -TUNNEL2_WILDCAM_RADIUS, +15.0f),
		D3DXVECTOR3(-TUNNEL2_WILDCAM_RADIUS, +TUNNEL2_WILDCAM_RADIUS, +15.0f),
		D3DXVECTOR3(-TUNNEL2_WILDCAM_RADIUS, -TUNNEL2_WILDCAM_RADIUS, +15.0f),
...
HRESULT CTunnel2::InitDeviceObjects()
{
	...
	if ( FAILED(hr = D3DXCreateTextureFromFile(g_pd3dDevice, _T("data\\bitmaps\\redwater.jpg"), &m_ptexTunnel)) )
	{
		DisplayErrorMsg2( D3DAPPERR_MEDIANOTFOUND, NULL );
		return hr;
	}
...
HRESULT FadeRect( D3DCOLOR dwColor, RECT rcArea, BOOL bUpdateRect )
{
	HRESULT hr = S_OK;

	if ( g_fDTime < g_fFadeStart || g_fDTime >= g_fFadeEnd )
	{
		g_bFading = FALSE;
		return S_OK;
	}
...
HRESULT ShowTexture( LPDIRECT3DTEXTURE8 pTexture, FLOAT fOffsetX, FLOAT fOffsetY, FLOAT fWidth, FLOAT fHeight, FLOAT fAlpha, FLOAT fTileFactor, BOOL bForeground )
{
	...
	g_pd3dDevice->SetRenderState( D3DRS_CULLMODE, D3DCULL_NONE );
	g_pd3dDevice->SetStreamSource( 0, g_pvbTexture, sizeof(TLTVERTEX) );
	g_pd3dDevice->SetVertexShader( D3DFVF_TLTVERTEX );
...
HRESULT ShowTextureVB( LPDIRECT3DTEXTURE8 pTexture, LPDIRECT3DVERTEXBUFFER8 pVB, INT numQuadsX, INT numQuadsY, FLOAT fAlpha=255.0f, FLOAT fTileFactor=1.0f )
{
	...
	g_pd3dDevice->SetStreamSource( 0, pVB, sizeof(TLTVERTEX) );
	g_pd3dDevice->SetVertexShader( D3DFVF_TLTVERTEX );
...
HRESULT SplashScreen2( FLOAT fLocalTime )
{
	...
	D3DCOLOR topVertCol[4] = 
	{
		D3DCOLOR_ARGB( 0xff, 0xff, 0xff, 0xff ),	// top left
		D3DCOLOR_ARGB( 0xff, 0xff, 0xff, 0xff ),	// top right
		D3DCOLOR_ARGB( 0x30, 0xff, 0xff, 0xff ),	// bottom right
		D3DCOLOR_ARGB( 0x30, 0xff, 0xff, 0xff ),	// bottom left
	};

	D3DCOLOR bleftVertCol[4] = 
	{
		D3DCOLOR_ARGB( 0x30, 0xff, 0xff, 0xff ),
		D3DCOLOR_ARGB( 0x30, 0xff, 0xff, 0xff ),
		D3DCOLOR_ARGB( 0x40, 0xff, 0xff, 0xff ),
	...
	frcTopSrc.left	  = LinearChange( 250.0f, 450.0f, 0.0f, fLocalTime, 25.0f );
	frcTopSrc.top	  = LinearChange( 100.0f, 150.0f, 0.0f, fLocalTime, 25.0f );

	frcBLeftSrc.left  = LinearChange( 620.0f, 580.0f, 0.0f, fLocalTime, 25.0f );
	frcBLeftSrc.top	  = LinearChange(  25.0f, 150.0f, 0.0f, fLocalTime, 25.0f );

	frcBRightSrc.left = LinearChange( 425.0f, 725.0f, 0.0f, fLocalTime, 25.0f );
	frcBRightSrc.top  = LinearChange(  50.0f,  30.0f, 0.0f, fLocalTime, 25.0f );
	...
	ShowTextureEx( g_ptexSplash2, frcTopSrc,	 topVertCol,	frcTop.left		* g_fScaleX, frcTop.top	   * g_fScaleY, frcTop.right	* g_fScaleX, frcTop.bottom	  * g_fScaleY, ALPHA_FULL );
	ShowTextureEx( g_ptexSplash2, frcBLeftSrc,	 bleftVertCol,	frcBLeft.left	* g_fScaleX, frcBLeft.top  * g_fScaleY, frcBLeft.right	* g_fScaleX, frcBLeft.bottom  * g_fScaleY, ALPHA_FULL );
	ShowTextureEx( g_ptexSplash2, frcBRightSrc, brightVertCol, frcBRight.left	* g_fScaleX, frcBRight.top * g_fScaleY, frcBRight.right	* g_fScaleX, frcBRight.bottom * g_fScaleY, ALPHA_FULL );
