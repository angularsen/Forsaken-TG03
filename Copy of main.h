...
#define D3DFVF_TLTVERTEX		( D3DFVF_XYZRHW | D3DFVF_DIFFUSE| D3DFVF_TEX1 )
...
struct TLTVERTEX	// Transformed, lit and textured
{
	FLOAT x, y, z;
	FLOAT rhw;
	DWORD diffuse;
...
struct TLTQUAD	// Used in ShowTextureVB. Describes a quad of TLTVERTEXes
{
	TLTVERTEX vertex[4];
};
...
protected:
	D3DLIGHT8				m_light;
	LPDIRECT3DTEXTURE8      m_ptexFirefly;
	LPDIRECT3DVERTEXBUFFER8 m_pVB;	// Vertex buffer for textured quad
...
protected:
	LPDIRECT3DTEXTURE8      m_ptexParticle;
	LPDIRECT3DVERTEXBUFFER8 m_pVB;	// Vertex buffer for textured quad
...
public:
	...
	LPDIRECT3DTEXTURE8		m_ptexMetaBall;
	LPDIRECT3DVERTEXBUFFER8 m_pVB;	// Vertex buffer for metaball
...
protected:
	D3DLIGHT8				m_light;
	LPDIRECT3DTEXTURE8      m_ptexText1;
	LPDIRECT3DTEXTURE8		m_ptexOverlay;
	LPDIRECT3DTEXTURE8		m_ptexOverlayText;

	LPDIRECT3DVERTEXBUFFER8 m_pVB;	// Vertex buffer for textured quads
	LPDIRECT3DINDEXBUFFER8	m_pIB;
...
public:
	...
	HRESULT GenerateTunnel			( LPDIRECT3DVERTEXBUFFER8 pVB, LPDIRECT3DINDEXBUFFER8 pIB, FLOAT fTunnelRadius, BOOL bGenerateWaypoint );
	HRESULT InitDeviceObjects		();
	HRESULT InvalidateDeviceObjects	();
    HRESULT Render					();
	HRESULT	RotateTexCoords			( LPDIRECT3DVERTEXBUFFER8 pVB, LPDIRECT3DINDEXBUFFER8 pIB, FLOAT fRot, FLOAT fAlpha=255.0f );
	
protected:
	LPDIRECT3DTEXTURE8		m_ptexTunnel;
	LPDIRECT3DTEXTURE8		m_ptexPlasma;
	LPDIRECT3DTEXTURE8		m_ptexPlasma2;
	LPDIRECT3DVERTEXBUFFER8	m_pvbTunnel1;
	LPDIRECT3DINDEXBUFFER8	m_pibTunnel1;
	LPDIRECT3DVERTEXBUFFER8	m_pvbTunnel2;
	LPDIRECT3DINDEXBUFFER8	m_pibTunnel2;
	LPDIRECT3DVERTEXBUFFER8	m_pvbTunnel3;
	LPDIRECT3DINDEXBUFFER8	m_pibTunnel3;
...
protected:
	LPDIRECT3DTEXTURE8		m_ptexTunnel;
	LPDIRECT3DVERTEXBUFFER8	m_pvbTunnel;
	LPDIRECT3DINDEXBUFFER8	m_pibTunnel;
...
protected:
	...
	LPDIRECT3DVERTEXBUFFER8	m_pvbStar;
	LPDIRECT3DINDEXBUFFER8	m_pibStar;
	LPDIRECT3DTEXTURE8		m_ptexStar;
	...
	CD3DFont*				g_pFont;
	DWORD					g_dwTextColor;
    LPDIRECT3DTEXTURE8		g_ptexBlender;		// Pointer to destination texture for blending textures to
	...
	LPDIRECT3DTEXTURE8		g_ptexScreenMask;	// Uses a texture with transparency, as to mask out part of the screen with the image
	LPDIRECT3DTEXTURE8		g_ptexScaleScreen;	// Low-res texture used to draw scene to before displaying on screen (to give a low-res effect)
	LPDIRECT3DTEXTURE8		g_ptexScreenBuffer;	// A screen buffer, to draw scenes to and add to backbuffer by a ShowTexture(). Transparent layers-effect.
	...
	LPDIRECT3DVERTEXBUFFER8 g_pvbFadeRect;      // Vertexbuffer for fade rect
	LPDIRECT3DVERTEXBUFFER8 g_pvbTexture;		// Pointer to vertexbuffer for ShowTexture()
	...
	// Loadscreen
	LPDIRECT3DTEXTURE8		g_ptexLoadScreen;

	// Scene 01
	CFireFlies				g_flyGroup1(0);

	LPDIRECT3DTEXTURE8		g_ptexWhatIf;		// Pointer to introtext
	LPDIRECT3DTEXTURE8		g_ptexWereToBlaim;		
	LPDIRECT3DTEXTURE8		g_ptexHumansAre;		
	LPDIRECT3DTEXTURE8		g_ptexPsychologically;		
	LPDIRECT3DTEXTURE8		g_ptexTheSun;
	LPDIRECT3DTEXTURE8		g_ptexTheEarth;
	LPDIRECT3DTEXTURE8		g_ptexWillBlaim;
	LPDIRECT3DTEXTURE8		g_ptexUsAlone;
	LPDIRECT3DTEXTURE8		g_ptexIAmFeeling;
	LPDIRECT3DTEXTURE8		g_ptexInAViolated;
	LPDIRECT3DTEXTURE8		g_ptexICanNoLonger;
	LPDIRECT3DTEXTURE8		g_ptexTheThings;
	LPDIRECT3DTEXTURE8		g_ptexIHaveForsaken;
	LPDIRECT3DTEXTURE8		g_ptexHeed;
	LPDIRECT3DTEXTURE8		g_ptexPresents;
	LPDIRECT3DTEXTURE8		g_ptexOurFirst;
	LPDIRECT3DTEXTURE8		g_ptexDemoEver;

	LPDIRECT3DTEXTURE8		g_ptexSplash1;		
	LPDIRECT3DTEXTURE8		g_ptexWater;		// Pointer to water texture
	LPDIRECT3DCUBETEXTURE8	g_ptexWaterEnvmap;	// Environmapped water texture
	CD3DMesh*				g_pmeshWater;
	LPDIRECT3DVERTEXBUFFER8 g_pvbWater;			// Vertexbuffer for water surface
	LPDIRECT3DINDEXBUFFER8	g_pibWater;			// Indexbuffer for water surface
	...
	// Scene 06
	LPDIRECT3DTEXTURE8		g_ptexSplash2;

	// Scene 07
	LPDIRECT3DTEXTURE8		g_ptexCredit1;
	LPDIRECT3DTEXTURE8		g_ptexCredit2;
	LPDIRECT3DTEXTURE8		g_ptexCredit3;
	LPDIRECT3DTEXTURE8		g_ptexHello;

	// Scene 08
	CScene08				g_scene08;

	// Scene 99
	CD3DMesh*				g_pmeshTorus;
	LPDIRECT3DVERTEXBUFFER8	g_pvbTorus;
	LPDIRECT3DINDEXBUFFER8	g_pibTorus;
	LPDIRECT3DTEXTURE8		g_ptexTorus;
...
// Tool-functions
	HRESULT BlendTexture		( LPDIRECT3DTEXTURE8 pSource1, LPDIRECT3DTEXTURE8 pSource2, LPDIRECT3DTEXTURE8 pDest, FRECT frcSource1, FRECT frcSource2, FLOAT fAlpha=255.0f, BOOL bReplace=FALSE );
	...
	HRESULT DisturbWater		( LPDIRECT3DVERTEXBUFFER8 pVB, LPDIRECT3DINDEXBUFFER8 pIB, DWORD dwFVF, WAVEDATA* wData, FLOAT fCurrentTime );
	HRESULT Fade				( D3DCOLOR dwColorStart, D3DCOLOR dwColorEnd, FLOAT fTStart, FLOAT fTCurrent, FLOAT fTEnd, FLOAT fTHold=0.0f );
	HRESULT FadeRect			( D3DCOLOR dwColor, RECT rcArea, BOOL bUpdateRect=FALSE );
...
inline 	FLOAT RandRange			( FLOAT low, FLOAT high);
	...
	HRESULT	SetDetailedVBRect	( LPDIRECT3DVERTEXBUFFER8 pVB, FLOAT fOffsetX, FLOAT fOffsetY, FLOAT fOffsetX2, FLOAT fOffsetY2, INT numQuadsX, INT numQuadsY, FLOAT fAlpha, FLOAT fTiled );
	...
	HRESULT SetVB3DPlaneRect	( LPDIRECT3DVERTEXBUFFER8 pVB, LPDIRECT3DINDEXBUFFER8 pIB, DWORD dwFVF, D3DXVECTOR3 vTopLeft, D3DXVECTOR3 vBottomRight, INT numVertsX, INT numVertsY, FLOAT fAlpha );
	HRESULT SetVBRect			( LPDIRECT3DVERTEXBUFFER8 pVB, RECT rcRect, D3DCOLOR dwColor=NULL );
	HRESULT ShowOrigin			();
	HRESULT ShowBlurTexture		( LPDIRECT3DTEXTURE8 pTexture, FLOAT fOffsetX, FLOAT fOffsetY, FLOAT fWidth, FLOAT fHeight, FLOAT fBlurIntensity, FLOAT fBlurSize, FLOAT fAlpha=255.0f, FLOAT fTileFactor=1.0f, BOOL bForeground=TRUE );
	HRESULT ShowTexture			( LPDIRECT3DTEXTURE8 pTexture, FLOAT fOffsetX, FLOAT fOffsetY, FLOAT fWidth, FLOAT fHeight, FLOAT fAlpha=255.0f, FLOAT fTileFactor=1.0f, BOOL bForeground=TRUE );
	HRESULT ShowTextureEx		( LPDIRECT3DTEXTURE8 pTexture, FRECT frcSrc, D3DCOLOR vertCol[4], FLOAT fOffsetX, FLOAT fOffsetY, FLOAT fWidth, FLOAT fHeight, FLOAT fAlpha=255.0f );
	HRESULT ShowTextureExTile	( LPDIRECT3DTEXTURE8 pTexture, D3DCOLOR vertCol[4], FLOAT fOffsetX, FLOAT fOffsetY, FLOAT fWidth, FLOAT fHeight, FLOAT fTileTU, FLOAT fTileTV, FLOAT fOffsetTU=0.0f, FLOAT fOffsetTV=0.0f, FLOAT fAlpha=255.0f );
	HRESULT ShowTextureVB		( LPDIRECT3DTEXTURE8 pTexture, LPDIRECT3DVERTEXBUFFER8 pVB, INT numQuadsX, INT numQuadsY, FLOAT fAlpha, FLOAT fTileFactor );
	HRESULT ShowTextureVBProj	( LPDIRECT3DTEXTURE8 pTexture, LPDIRECT3DVERTEXBUFFER8 pVB, LPDIRECT3DINDEXBUFFER8 pIB, D3DXVECTOR3 vTopLeft, D3DXVECTOR3 vBottomRight, INT numVertsX, INT numVertsY, FLOAT fAlpha );
	...
	HRESULT AScene99( FLOAT fTime );	// Torus
